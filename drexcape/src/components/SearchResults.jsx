...
const SearchResults = () => {
  ...
  useEffect(() => {
    ...
    const generateItinerary = async (currentRetryCount = 0, currentRetryDelay = retryDelay) => {
      try {
        ...
        if (data.error && data.error.code === 503) {
          setError(`AI model is temporarily overloaded. Retrying in ${currentRetryDelay/1000} seconds... (Attempt ${currentRetryCount + 1}/3)`);
          
          if (currentRetryCount < 2) {
            setTimeout(() => {
              generateItinerary(currentRetryCount + 1, currentRetryDelay * 2);
            }, currentRetryDelay);
            return;
          } else {
  ...
  const handleViewDetails = async (itinerary, index) => {
    ...
    try {
      ...
      if (!isUserLoggedIn) {
        ...
        setIsNavigating(false); // Reset immediately for login flow
        return;
      }
